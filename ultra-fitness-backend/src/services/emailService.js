...
const sendEmail = async (options) => {
    const message = {
        from: process.env.EMAIL_FROM || 'Mother Fitness <noreply@motherfitness.com>',
        to: options.to,
...
const getExpiryTemplate = (name, plan, validity) => {
    ...
    const subject = `⚠️ Membership Expiring Soon - Mother Fitness`;

    const text = `Hello ${name},\n\nYour ${plan} membership at Mother Fitness is expiring on ${dateStr}.\n\nPlease renew your membership to continue enjoying our facilities.\n\nRegards,\nMother Fitness Team`;

    const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
        <h2 style="color: #e74c3c;">Membership Expiring Soon</h2>
        <p>Hello <strong>${name}</strong>,</p>
        <p>This is a friendly reminder that your <strong>${plan}</strong> membership at Mother Fitness is expiring on:</p>
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; font-size: 12px; color: #888;">
            <p>Regards,<br>Mother Fitness Team</p>
        </div>
    </div>
    `;
...
const getWelcomeTemplate = (name, plan) => {
    const subject = `🎉 Welcome to Mother Fitness!`;

    const text = `Hello ${name},\n\nWelcome to Mother Fitness! We are excited to have you on board with our ${plan} plan.\n\nGet ready to achieve your fitness goals!\n\nRegards,\nMother Fitness Team`;

    const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
        <h2 style="color: #2ecc71;">Welcome to Mother Fitness!</h2>
        <p>Hello <strong>${name}</strong>,</p>
